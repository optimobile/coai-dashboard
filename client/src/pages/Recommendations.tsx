...
export default function Recommendations() {
  ...
  const dismissedIds = new Set(dismissedData?.dismissedIds || []);
  const snoozedIds = new Set(dismissedData?.snoozedIds || []);
  ...
  const submitFeedback = async () => {
    ...
    await trackMutation.mutateAsync({
      ...
      recommendationType: feedbackDialog.recType,
  ...
  const submitSnooze = async () => {
    ...
    await trackMutation.mutateAsync({
      ...
      recommendationType: snoozeDialog.recType,
      action: 'snoozed',
      snoozeDays: selectedSnoozeDays,
      metadata: {
        title: snoozeDialog.title,
      },
    });
            ...
            <Card>
              ...
              <CardContent>
                {analytics && analytics.length > 0 ? (
                  <div className="space-y-4">
                    {analytics.map((month: any) => (
                      <div key={month.id} className="p-4 rounded-lg bg-muted/50">
                        <div className="flex items-center justify-between mb-3">
                          <span className="font-medium">{month.period}</span>
                          <span className="text-sm text-muted-foreground">
                            {month.totalViewed} viewed
                          </span>
                        </div>
                        <div className="grid grid-cols-4 gap-4 text-center">
                          <div>
                            <p className="text-2xl font-bold text-green-400">{month.totalImplemented}</p>
                            <p className="text-xs text-muted-foreground">Implemented</p>
                          </div>
                          <div>
                            <p className="text-2xl font-bold text-red-400">{month.totalDismissed}</p>
                            <p className="text-xs text-muted-foreground">Dismissed</p>
                          </div>
                          <div>
                            <p className="text-2xl font-bold text-yellow-400">{month.totalSnoozed}</p>
