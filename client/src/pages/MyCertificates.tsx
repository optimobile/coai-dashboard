...
export default function MyCertificates() {
  ...
  const generateCertificateMutation = trpc.certificates.generate.useMutation({
    onSuccess: (data) => {
      if (data.success && data.pdfData) {
        ...
        link.href = url;
        link.download = `COAI-Certificate-${data.certificateId}.pdf`;
  ...
  const courseCertList = courseCertificates?.certificates || [];
        ...
        <Dialog open={!!selectedWatchdogCert} onOpenChange={() => setSelectedWatchdogCert(null)}>
          <DialogContent className="max-w-2xl">
            ...
            {selectedWatchdogCert && (
              <Certificate
                certificateNumber={selectedWatchdogCert.certificateNumber}
                userName="Certified Analyst"
                testName="Watchdog Analyst Certification Exam"
                score={85}
                issuedDate={new Date(selectedWatchdogCert.issuedAt)}
                expiryDate={new Date(selectedWatchdogCert.expiresAt)}
              />
            )}
          </DialogContent>
        </Dialog>
